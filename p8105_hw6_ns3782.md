...
To assess these models, I used cross-validation with 100 iterations to
...
```
    ...
    ## The first warning was:
    ## ℹ In argument: `rmse_main = map2_dbl(main_fit, test, rmse)`.
    ## Caused by warning in `predict.lm()`:
    ## ! prediction from rank-deficient fit; attr(*, "non-estim") has doubtful cases
    ## ℹ Run `dplyr::last_dplyr_warnings()` to see the 2 remaining warnings.

I first compared my model to a simpler model using only length at birth
and gestational age as predictors.

The cross-validation process involved creating a 100 training/testing
splits, fitting both models on each training set, and then computing
prediction error on each testing set.
