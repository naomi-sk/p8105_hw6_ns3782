...
To assess these models, I used cross-validation with 100 iterations to estimate prediction accuracy.

```{r}

# First comparison: Main model vs Length-Gestational model
...
I first created 100 training and testing sets using cross-validation. 

```{r}

# Compare main model to length at birth/gestational age model

cv_results_1 = 
  cv_splits %>% 
  mutate(
    main_fit = map(train, ~lm(bwt ~ babysex + bhead + blength + delwt + fincome + 
                             frace + gaweeks + menarche + mheight + momage + 
                             mrace + parity + ppwt + smoken + wtgain, data = .x)),
    length_gest_fit = map(train, ~lm(bwt ~ blength + gaweeks, data = .x))
  ) %>% 
  mutate(
    rmse_main = map2_dbl(main_fit, test, rmse),
    rmse_length_gest = map2_dbl(length_gest_fit, test, rmse)
  )

```

I first compared my model to a simpler model using only length at birth and gestational age as predictors.

The cross-validation process involved creating a 100 training/testing splits, fitting both models on each training set, and then computing prediction error on each testing set.